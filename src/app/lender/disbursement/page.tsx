    ...
    batchApprove.mutate(undefined, {
      onSuccess: (res) => {
        ...
        if (res.disbursed.length > 0) {
          toast.success(`Disbursed ${res.disbursed.length} loan${res.disbursed.length === 1 ? "" : "s"}`);
        }
        // Always show WHY a loan failed, not just how many — "insufficient
        // balance" is self-explanatory, but "this borrower already has an
        // active loan elsewhere" genuinely needs the reason surfaced, and a
        // mixed batch (some sent, some not) would otherwise hide it behind
        // just a count.
                ...
                {pending.map((loan) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-[#1B2B3A] dark:text-white truncate flex items-center gap-1.5">
                        {loan.borrower_name ?? "—"}
                        {loan.borrower_has_active_loan_elsewhere && (
            ...
            {!selected ? (
              ...
              <>
                <div className="flex items-center justify-between mb-4">
                  ...
                  {selected.borrower_has_active_loan_elsewhere ? (
                    <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-600 border border-amber-200 dark:border-amber-800 dark:text-amber-400">
                      On Hold
                    </span>
                  ) : (
                ...
                {selected.borrower_has_active_loan_elsewhere && (
                  <div className="flex items-start gap-2.5 rounded-xl border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-900/20 px-4 py-3 mb-4">
                    <AlertTriangle className="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
                    <p className="text-xs text-amber-700 dark:text-amber-400">
                      This borrower already has an active loan with another lender. They can&apos;t
                      be disbursed a second loan until that one is fully repaid.
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={approve.isPending || selected.borrower_has_active_loan_elsewhere}
                    title={
                      selected.borrower_has_active_loan_elsewhere
                        ? "Blocked — this borrower already has an active loan elsewhere"
                        : undefined
                    }
                    className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-[#C4A55A] text-white text-sm font-semibold hover:bg-[#b3944a] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Send className="w-4 h-4" />
