      ...
      <CardContent className="p-6 space-y-4">
        ...
        <div className="space-y-3 text-sm">
          <div className="flex justify-between">
            ...
            <span className="font-semibold text-[#1B2B3A]">UGX {amount.toLocaleString()}</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold text-[#1B2B3A]">{duration} month{duration !== 1 ? "s" : ""}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Total Interest</span>
            <span className="font-semibold text-[#2BB5A0]">UGX {totalInterest.toLocaleString()}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Monthly Payment</span>
            <span className="font-semibold text-[#1B2B3A]">UGX {Math.round(monthly).toLocaleString()}</span>
          </div>
          <div className="h-px bg-gray-200" />
          <div className="flex justify-between">
            <span className="font-bold text-[#1B2B3A]">Total Repayable</span>
            <span className="font-bold text-[#2BB5A0]">UGX {totalRepayable.toLocaleString()}</span>
          </div>
        </div>
        <p className="text-xs text-gray-400 mt-2">Applying to: James Mugisha&apos;s offer</p>
      </CardContent>
    </Card>
  );
}
...
function Step1({
  amount, setAmount, duration, setDuration, loanType, setLoanType, purpose, setPurpose,
}: {
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-[#2BB5A0]">
              <span className="px-4 py-3 bg-gray-50 text-gray-500 text-sm font-medium border-r border-gray-300">UGX</span>
              <input
                type="number"
                value={amount}
...
function UploadZone({ label, uploaded, filename }: { label: string; uploaded?: boolean; filename?: string }) {
  return (
    <div className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors ${uploaded ? "border-[#2BB5A0] bg-[#2BB5A0]/5" : "border-gray-300 hover:border-[#2BB5A0] bg-gray-50"}`}>
      {uploaded ? (
        <div className="flex items-center justify-center gap-2">
          <Check className="w-5 h-5 text-[#2BB5A0]" />
          <span className="text-sm font-medium text-[#1B2B3A]">{filename}</span>
          <span className="text-xs bg-[#2BB5A0] text-white px-2 py-0.5 rounded-full">Uploaded</span>
        </div>
      ) : (
  ...
  return (
    <Card className="bg-white border border-gray-200">
      <CardContent className="p-6 space-y-6">
        ...
        <div>
          <label className="block text-sm font-semibold text-[#1B2B3A] mb-2">National ID</label>
          <UploadZone label="Upload National ID" uploaded filename="NIN-CF12345678.pdf" />
        </div>

        <div>
          <label className="block text-sm font-semibold text-[#1B2B3A] mb-2">Bank Statement (3 months)</label>
          <UploadZone label="Upload Bank Statement" />
        </div>

        <div>
          <label className="block text-sm font-semibold text-[#1B2B3A] mb-2">Business Registration / Payslip</label>
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="border-2 border-[#2BB5A0] rounded-lg p-4 flex items-center justify-between bg-[#2BB5A0]/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-[#1B2B3A] text-sm">Joseph Kato</p>
              <p className="text-xs text-gray-500">+256 700 123 456 · Friend</p>
            </div>
          </div>
          <span className="text-xs bg-[#2BB5A0] text-white px-3 py-1 rounded-full font-medium">Confirmed</span>
        </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-[#2BB5A0]">
                <span className="px-4 py-2.5 bg-[#2BB5A0] text-white text-sm font-medium">+256</span>
                <input
                  type="tel"
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">NIN</label>
              <input
                type="text"
...
function Step4({ amount, duration }: { amount: number; duration: number }) {
  ...
  return (
    <Card className="bg-white border border-gray-200">
      <CardContent className="p-6 space-y-6">
        <h2 className="text-xl font-bold text-[#1B2B3A]">Review &amp; Submit</h2>

        <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
          <div className="w-10 h-10 rounded-full bg-[#1B2B3A] text-white flex items-center justify-center text-sm font-bold">JM</div>
          <div>
        ...
        <table className="w-full text-sm">
          <tbody className="divide-y divide-gray-100">
            {[
              ...
              ["Total Repayable", `UGX ${totalRepayable.toLocaleString()}`, true],
              ["Guarantors", "1 confirmed", false],
            ].map(([label, value, highlight], i) => (
              <tr key={i}>
                <td className="py-3 text-gray-500">{label}</td>
                <td className={`py-3 font-semibold text-right ${highlight ? "text-[#2BB5A0]" : "text-[#1B2B3A]"}`}>
                  {value}
                </td>
              </tr>
            ))}
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-800">
          By submitting this application, you agree to WeLend&apos;s terms and confirm that all information provided is accurate and complete.
        </div>
      </CardContent>
    </Card>
  );
...
function SuccessView() {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center space-y-6">
      ...
      <div>
        <h2 className="text-2xl font-bold text-[#1B2B3A]">Application Submitted!</h2>
        <p className="text-gray-500 mt-1">Reference: <span className="font-semibold text-[#1B2B3A]">#APP-20240091</span></p>
      </div>
      ...
      <div className="w-full max-w-lg">
        <div className="flex items-center justify-between">
          {["Submitted", "Lender Review", "Approval", "Disbursement", "Repayment Begins"].map((label, idx) => (
            <div key={label} className="flex flex-col items-center gap-1 flex-1">
              ...
              <span className={`text-xs text-center leading-tight ${idx === 0 ? "text-[#2BB5A0] font-medium" : "text-gray-400"}`}>
                {label}
              </span>
            </div>
          ))}
      ...
      <div className="flex gap-3 mt-4">
        <Button asChild variant="outline" className="border-[#1B2B3A] text-[#1B2B3A]">
          <a href="/dashboard">Back to Dashboard</a>
        </Button>
        <Button asChild className="bg-[#2BB5A0] hover:bg-[#239E8C] text-white">
          <a href="/dashboard/my-requests">View My Applications</a>
        </Button>
      </div>
    </div>
  );
...
export default function ApplyPage() {
  ...
  const stepTitles = ["Apply — Loan Details", "Apply — Documents", "Apply — Guarantors", "Apply — Review"];
  const nextLabels = ["Next: Documents \u2192", "Next: Guarantors \u2192", "Next: Review \u2192", "Submit Application"];
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <BorrowerPageHeader title={submitted ? "Apply — Submitted" : stepTitles[currentStep - 1]} />
      ...
      {submitted ? (
        ...
        <>
          ...
          <div className="flex justify-between pt-4">
            <Button variant="outline" onClick={handleBack} className="border-gray-300 text-[#1B2B3A]">
              Back
            </Button>
            <Button onClick={handleNext} className="bg-[#2BB5A0] hover:bg-[#239E8C] text-white">
